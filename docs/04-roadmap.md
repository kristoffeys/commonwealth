...
front-loads the two things that are both hard *and* differentiating (concurrency-safe
...
1. **License** — MIT vs. Apache-2.0 (patent grant). *(M0/M5)*
2. **Review gate default** — PR-per-promotion vs. lightweight in-repo queue. *(M3)*
3. **Embeddings** — local model vs. hosted for the index. *(M1/M3)*
4. **Relevance-injection budget** — ranking + token budget at SessionStart. *(M3)*
5. **Managed-settings distribution** — verify exact Claude Code plugin-push path. *(M4)*
6. **Hosted daemon topology** — per-user vs. multi-tenant for the paid tier. *(M5)*
7. **Repo granularity** — per-project repos vs. monorepo-of-brains at scale. *(M0/M4)*
