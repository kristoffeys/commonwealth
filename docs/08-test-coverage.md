...
| True simultaneous writers (real race, not turn-taking) | `packages/curate/src/capture.ts` (contributor person resolution) | `packages/curate/test/capture-autopromote.test.ts` (`converges concurrent name-only and email-backed first writes`, uses `Promise.all`) | ✅ Full | The one place in the suite that fires two writers with genuine `Promise.all` concurrency rather than sequential turns. Everything else in this table is turn-taking simulation of the *outcome* of a race (see cross-cutting note). |
...
exercised is `capture-autopromote.test.ts`'s contributor-identity test. **What a stronger
